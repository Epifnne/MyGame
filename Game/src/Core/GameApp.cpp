...
bool GameApp::Initialize(Runtime::Core::Engine& engine) {
    ...
    m_camera.SetTarget(glm::vec3(0.9f, -0.2f, 0.0f));
    ...
    groundColliderDesc.material.restitution = 0.12f;
    groundColliderDesc.material.dynamicFriction = 0.8f;
    groundColliderDesc.material.staticFriction = 0.9f;
    ...
    ballColliderDesc.material.restitution = 0.74f;
    ballColliderDesc.material.dynamicFriction = 0.35f;
    ballColliderDesc.material.staticFriction = 0.55f;
...
void GameApp::Update(float dt, const Runtime::Core::Input& input) {
    ...
    if (ballBody) {
        ...
        if (input.IsKeyDown(Runtime::Core::Key_W)) {
            desiredHorizontalVelocity.z -= 2.4f;
        }
        if (input.IsKeyDown(Runtime::Core::Key_S)) {
            desiredHorizontalVelocity.z += 2.4f;
        }
        if (input.IsKeyDown(Runtime::Core::Key_A)) {
            desiredHorizontalVelocity.x -= 2.4f;
        }
        if (input.IsKeyDown(Runtime::Core::Key_D)) {
            desiredHorizontalVelocity.x += 2.4f;
        }

        glm::vec3 currentVelocity = ballBody->LinearVelocity();
        currentVelocity.x = desiredHorizontalVelocity.x;
        currentVelocity.z = desiredHorizontalVelocity.z;
        ballBody->SetLinearVelocity(currentVelocity);
    }
    ...
    for (const auto& contact : contacts) {
        ...
        const float penetrationSquash = std::clamp(contact.point.penetration * 2.8f, 0.0f, 0.55f);
        const float impactSquash = std::clamp(impactSpeed * 0.04f, 0.0f, 0.55f);
        targetSquash = std::max(targetSquash, penetrationSquash + impactSquash);
    }
    targetSquash = std::clamp(targetSquash, 0.0f, 0.65f);

    if (ballBody) {
        const float velocityY = ballBody->LinearVelocity().y;
        const bool bouncedUp = prevVelocityY < -0.3f && velocityY > 0.15f;
        if (bouncedUp) {
            const float bouncePulse = std::clamp((-prevVelocityY) * 0.055f, 0.0f, 0.65f);
    ...
    if (renderer) {
        ...
        if (m_mesh && m_ballMesh) {
            ...
            if (ballBody) {
                ...
                const float anchorOffset = m_ballRadius * (1.0f - yScale) * 0.82f;
